...
function makeRequest(method: string, body?: unknown): NextRequest {
  ...
  return new NextRequest(`http://localhost/api/migraine-symptom-types/${TYPE_ID}`, init)
}
...
describe('PUT /api/migraine-symptom-types/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('updates type name', async () => {
    ...
    db.migraineSymptomType.findFirst.mockResolvedValue({
      id: TYPE_ID,
      name: 'Old',
      userId: 'test-user-id-123',
    } as never)
    db.migraineSymptomType.update.mockResolvedValue({
      id: TYPE_ID,
      name: 'Updated',
      userId: 'test-user-id-123',
    } as never)
  ...
  it('returns 404 when type not found', async () => {
    mockRequireAuth.mockResolvedValue(mockAuthContext())
    db.migraineSymptomType.findFirst.mockResolvedValue(null)
  ...
  it('returns 409 for duplicate name', async () => {
    ...
    db.migraineSymptomType.update.mockRejectedValue({ code: 'P2002' })
...
describe('DELETE /api/migraine-symptom-types/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('deletes type owned by user', async () => {
    ...
    db.migraineSymptomType.delete.mockResolvedValue({} as never)
