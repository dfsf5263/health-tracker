...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch migraine symptom types' }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      console.error('Validation error creating migraine symptom type:', {
        error: error.issues,
        requestBody: body,
        userId,
        userDbId: user?.id,
        endpoint: 'POST /api/migraine-symptom-types',
      })
    ...
    console.error('Error creating migraine symptom type:', {
      error,
      requestBody: body,
      userId,
      userDbId: user?.id,
    ...
    return NextResponse.json({ error: 'Failed to create migraine symptom type' }, { status: 500 })
  }
}
