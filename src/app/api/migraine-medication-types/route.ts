...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch migraine medication types' },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      console.error('Validation error creating migraine medication type:', {
        error: error.issues,
        requestBody: body,
        userId,
        userDbId: user?.id,
        endpoint: 'POST /api/migraine-medication-types',
      })
      return NextResponse.json(
    ...
    console.error('Error creating migraine medication type:', {
      error,
      requestBody: body,
      userId,
      userDbId: user?.id,
    ...
    return NextResponse.json(
      { error: 'Failed to create migraine medication type' },
      { status: 500 }
    )
  }
}
