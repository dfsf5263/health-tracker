...
function makeRequest(method: string, body?: unknown): NextRequest {
  ...
  return new NextRequest(`http://localhost/api/migraine-medication-types/${TYPE_ID}`, init)
}
...
describe('PUT /api/migraine-medication-types/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('updates type name', async () => {
    ...
    db.migraineMedicationType.findFirst.mockResolvedValue({
      id: TYPE_ID,
      name: 'Old',
      userId: 'test-user-id-123',
    } as never)
    db.migraineMedicationType.update.mockResolvedValue({
      id: TYPE_ID,
      name: 'Updated',
      userId: 'test-user-id-123',
    } as never)
  ...
  it('returns 404 when type not found', async () => {
    mockRequireAuth.mockResolvedValue(mockAuthContext())
    db.migraineMedicationType.findFirst.mockResolvedValue(null)
  ...
  it('returns 409 for duplicate name', async () => {
    ...
    db.migraineMedicationType.update.mockRejectedValue({ code: 'P2002' })
...
describe('DELETE /api/migraine-medication-types/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('deletes type owned by user', async () => {
    ...
    db.migraineMedicationType.delete.mockResolvedValue({} as never)
