...
function makeRequest(method: string, body?: unknown): NextRequest {
  ...
  return new NextRequest(`http://localhost/api/normal-physical-types/${TYPE_ID}`, init)
}
...
describe('PUT /api/normal-physical-types/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('updates type name', async () => {
    ...
    db.normalPhysicalType.update.mockResolvedValue({
      id: TYPE_ID,
      name: 'Updated',
      userId: 'test-user-id-123',
    } as never)
  ...
  it('returns 409 for duplicate name', async () => {
    ...
    db.normalPhysicalType.update.mockRejectedValue({ code: 'P2002' })
...
describe('DELETE /api/normal-physical-types/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
