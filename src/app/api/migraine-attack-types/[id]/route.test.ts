...
function makeRequest(method: string, body?: unknown): NextRequest {
  ...
  return new NextRequest(`http://localhost/api/migraine-attack-types/${TYPE_ID}`, init)
}
...
describe('PUT /api/migraine-attack-types/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('updates type name', async () => {
    ...
    db.migraineAttackType.findFirst.mockResolvedValue({
      id: TYPE_ID,
      name: 'Old',
      userId: 'test-user-id-123',
    } as never)
    db.migraineAttackType.update.mockResolvedValue({
      id: TYPE_ID,
      name: 'Updated',
      userId: 'test-user-id-123',
    } as never)
  ...
  it('returns 404 when type not found', async () => {
    mockRequireAuth.mockResolvedValue(mockAuthContext())
    db.migraineAttackType.findFirst.mockResolvedValue(null)
  ...
  it('returns 409 for duplicate name', async () => {
    ...
    db.migraineAttackType.update.mockRejectedValue({ code: 'P2002' })
...
describe('DELETE /api/migraine-attack-types/[id]', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('deletes type owned by user', async () => {
    ...
    db.migraineAttackType.delete.mockResolvedValue({} as never)
