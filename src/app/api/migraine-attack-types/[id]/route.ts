...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!existingType) {
      return NextResponse.json({ error: 'Migraine attack type not found' }, { status: 404 })
    }
    ...
    if (error instanceof z.ZodError) {
      console.error('Validation error updating migraine attack type:', {
        error: error.issues,
        requestBody: body,
        typeId: id,
        userId,
        userDbId: user?.id,
        endpoint: `PUT /api/migraine-attack-types/${id}`,
      })
    ...
    console.error('Error updating migraine attack type:', {
      error,
      requestBody: body,
      typeId: id,
      userId,
    ...
    return NextResponse.json({ error: 'Failed to update migraine attack type' }, { status: 500 })
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error('Error deleting migraine attack type:', {
      error,
      typeId: id,
      userId,
      endpoint: `DELETE /api/migraine-attack-types/${id}`,
    })
    return NextResponse.json({ error: 'Failed to delete migraine attack type' }, { status: 500 })
  }
}
