...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch migraine trigger types' }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      console.error('Validation error creating migraine trigger type:', {
        error: error.issues,
        requestBody: body,
        userId,
        userDbId: user?.id,
        endpoint: 'POST /api/migraine-trigger-types',
      })
    ...
    console.error('Error creating migraine trigger type:', {
      error,
      requestBody: body,
      userId,
      userDbId: user?.id,
    ...
    return NextResponse.json({ error: 'Failed to create migraine trigger type' }, { status: 500 })
  }
}
