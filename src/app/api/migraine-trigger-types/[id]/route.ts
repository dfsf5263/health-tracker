...
export const PUT = withApiLogging(
  async (request: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
    ...
    try {
      ...
      if (!existingType) {
        return ApiError.notFound('Migraine trigger type', requestId)
      }
      ...
      if (error instanceof z.ZodError) {
        await logApiError({
          request,
          error,
          operation: 'updating migraine trigger type',
          context: {
            requestBody: body,
            typeId: id,
            userId,
      ...
      return ApiError.internal('update migraine trigger type', requestId)
    }
  }
)
...
export async function DELETE(
  ...
  try {
    ...
    await logApiError({
      request,
      error,
      operation: 'deleting migraine trigger type',
      context: {
        typeId: id,
        userId,
      },
    ...
    return ApiError.internal('delete migraine trigger type', requestId)
  }
}
